...
document.addEventListener('DOMContentLoaded', function() {
    ...
    addButton.addEventListener('click', function() {
      ...
      // Add d-flex CLASS
        fieldDiv.classList.add('d-flex');
      // Add style of gap 5px
    ...
    fieldsContainer.addEventListener('click', function(event) {
      if (event.target.classList.contains('remove')) {
        ...
        const keyInput = field.querySelector('.key').value;
        
        // Remove from UI
        field.remove();
        
        // Remove from data
        const index = data.findIndex(item => item.key === keyInput);
        if (index !== -1) {
          data.splice(index, 1);
        }
    ...
    // When form with id settings_save_form is submitted, intercept the submission and save the data
    document.getElementById('settings_save_form').addEventListener('submit', function(event) {
    ...
    // On typing on each field, auto save
    addButton.addEventListener('click', updateData);

    // Add function
    function updateData() {
      const fields = document.querySelectorAll('.field');
      data.length = 0; // Clear data array
      
      fields.forEach(function(field) {
        const keyInput = field.querySelector('.key');
        const valueInput = field.querySelector('.value');
        const key = keyInput.value.trim(); // Trim to remove whitespace
        const value = valueInput.value;

        if (key && value && !data.some(item => item.key === key)) {
          data.push({ key: key, value: value });
        }
      });

      document.getElementById('api_variables').value = JSON.stringify(data);
    }
});
