...
$(document).on("click", ".cuModalBtn", function () {
    ...
    if (resource) {
        ...
        let fields = cuModal.find("input, select, textarea, datetime-local");
        let fieldName;
        ...
        fields.each(function (index, element) {
            ...
            if (fieldName != "_token" && resource[fieldName]) {
                ...
                } else if($(element).hasClass('select2-tags')) {
                    $(element).val("").change();
                    var option = "";
                    let string = resource[fieldName];
                    const split_string = string.split("|");
                    split_string.forEach(function (data){
                        option = option.concat("<option selected>"+ data +"</option>");
                    })
                    $(element).append(option).change();
                } else {
            ...
            // Update checkbox checked prop 
            if (element.type == "checkbox") {
                if (resource[fieldName] == 'yes') {
