...
<div class="m-0 p-2 {{ ($showBetSlip) ? 'col-xl-4 col-lg-6 col-md-12 col-sm-12' : 'col-xl-4 col-lg-4 col-md-4 col-sm-12'}} game-container ip_favorites_h2h_wrap" wire:key="{{ $game->id }}">
                ...
                <div class="p-4">
                    @if($game->stat)
                        @foreach($game->stat as $stat)
                            <p class="stat-title font-white m-0 p-0 text-center">{{ __($stat->display_name) }}</p>
                        @endforeach
                    @endif
                    <div class="row flex-column  gap-3">
                        <div class="col-12 position-relative rival-box-container my-1 p-0 {{ (isset($betSlipCart[$game->id]) && $betSlipCart[$game->id] == $game->player_one_id) ? 'selected-cards-box-container' : '' }}">
                    ...
                    @endauth
                        ...
                        <div class="col-12 versus-container">
                            <img class="versus" src="{{ asset('assets/templates/basic/images/icons/versus.svg') }}" />
                        </div>
                        <div wire:click="selectH2h({{ $game->id }}, '{{ $game->player_two_id }}')" class="col-12 position-relative rival-box-container my-1 p-0 {{ (isset($betSlipCart[$game->id]) && $betSlipCart[$game->id] == $game->player_two_id) ? 'selected-cards-box-container' : '' }}">
                            <div class="expand-button">
                                <img class="graph-icon" src="{{ asset('assets/templates/basic/images/icons/magnifying-glass.svg') }}" />
                            </div>
