...
class CFB_Helper
{
                ...
                foreach ($games as $game) {
                    ...
                    $schedule_result->updateOrCreate(
                        ...
                        [
                            ...
                            'home_points' => $game['scoring']['home_points'] ?? null,
                            'away_points' => $game['scoring']['away_points'] ?? null,
        ...
        if ($this->apiProvider == 'sportsradar') {
            ...
            foreach ($home_players as $stat => $statData) {
                ...
                foreach ($statData['players'] as $player) {
                    ...
                    $mergedStats[$player['id']]['sr_id'] = $player['sr_id'] ?? null;
                    $mergedStats[$player['id']]['jersey'] = $player['jersey'] ?? null;
                    $mergedStats[$player['id']]['position'] = $player['position'] ?? null;
    ...
    public function updatePlayerStats($data)
    {
        ...
        if ($this->apiProvider == 'sportsradar') {
            ...
            // Possible Stats
            $stat_keys = $data['res']['record'] ?? [];

            // Get only the keys
            $stat_keys = array_keys($stat_keys);
            ...
            foreach ($players as $player) {
                ...
                // Loop through all keys in players and match with stat keys and add to statistic
                foreach ($player as $key => $value) {
                    if (in_array($key, $stat_keys)) {
